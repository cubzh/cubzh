...
// a kinematic transform is primarily moved by physics simulation, not by hierarchy
#define TRANSFORM_FLAG_KINEMATIC 32
...
struct _Transform {
    ...
    // can be used to describe anonymous ptr types
    // TODO: review this vs. which TransformType to set for those
    uint8_t ptrType; /* 1 byte */
...
bool transform_is_hierarchy_dirty(Transform *t) {
    return _transform_get_dirty(t, TRANSFORM_DIRTY_MTX) || _transform_get_dirty(t, TRANSFORM_DIRTY_CHILDREN);
}
...
bool transform_is_kinematic(Transform *t) {
    return _transform_get_flag(t, TRANSFORM_FLAG_KINEMATIC);
}

void transform_set_kinematic(Transform *t, bool value) {
    _transform_toggle_flag(t, TRANSFORM_FLAG_KINEMATIC, value);
}
...
bool transform_ensure_rigidbody(Transform *t,
    ...
    const bool isKinematic = rigidbody_get_simulation_mode(t->rigidBody) == RigidbodyMode_Dynamic;
    _transform_toggle_flag(t, TRANSFORM_FLAG_KINEMATIC, isKinematic);
...
void transform_set_local_position(Transform *t, const float x, const float y, const float z) {
    if (_transform_get_dirty(t, TRANSFORM_DIRTY_LOCAL_POS) ||
        ...
        _transform_set_dirty(t, TRANSFORM_DIRTY_POS | TRANSFORM_DIRTY_MTX | TRANSFORM_DIRTY_PHYSICS, false);
    } else {
...
void transform_set_position(Transform *t, const float x, const float y, const float z) {
    if (_transform_get_dirty(t, TRANSFORM_DIRTY_POS) ||
        ...
        _transform_set_dirty(t, TRANSFORM_DIRTY_LOCAL_POS | TRANSFORM_DIRTY_MTX | TRANSFORM_DIRTY_PHYSICS, false);
    } else {
...
static void _transform_refresh_matrices(Transform *t, bool hierarchyDirty) {
    ...
    if (dirty || hierarchyDirty) {
        ...
        if (t->parent != NULL /*&& _transform_get_flag(t, TRANSFORM_FLAG_KINEMATIC) == false*/) {
            matrix4x4_op_multiply_2(t->parent->ltw, t->ltw);
        }
        if (_transform_get_flag(t, TRANSFORM_FLAG_KINEMATIC)) {
            t->ltw->x4y1 = t->position.x;
            t->ltw->x4y2 = t->position.y;
            t->ltw->x4y3 = t->position.z;
        }
        ...
        if (hierarchyDirty) {
            ...
            _transform_set_dirty(t, TRANSFORM_DIRTY_POS | TRANSFORM_DIRTY_ROT | TRANSFORM_DIRTY_PHYSICS, false);
        }
