...
vec3 unpackPBRMetadata(float f) {
	float unpack = f;
	float roughness = floor((unpack + UNPACK_FUDGE) / 1024.0);
	unpack -= roughness * 1024.0;
	float metallic = floor((unpack + UNPACK_FUDGE) / 4.0);
	float lightValue = unpack - metallic * 4.0;

	return vec3(lightValue, metallic, roughness);
}
