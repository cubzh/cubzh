...
void main() {
    ...
    vec3 normal = mix(v_normal, decodeNormal(texture2D(s_fb2, v_uv).xyz), metadata[MESH_METADATA_NORMALFLAG]);
    ...
    vec3 emissive = mix(vec3_splat(0.0), texture2D(s_fb4, v_uv).xyz, metadata[MESH_METADATA_EMISSIVEFLAG]);

    float unlit = mix(LIGHTING_LIT_FLAG, LIGHTING_UNLIT_FLAG, u_unlit);
    emissive *= unpackFloatToRgb(u_emissive);
#else
