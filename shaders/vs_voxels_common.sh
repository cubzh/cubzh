...
void main() {
	...
	vec3 cnormal = normalize(mul(u_modelViewProj, getVertexNormal(face)).xyz);
	clip.xyz += cnormal * u_outlineThickness;
#endif
	...
	vec3 wnormal = normalize(mul(u_model[0], getVertexNormal(face)).xyz);
