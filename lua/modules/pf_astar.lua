...
pf_astar.newPathfinder = function(self, step, useDiagonals, useCache)
  ...
  pathfinder.find = function(self, startX, startZ, endX, endZ, map, testFunction, useDiagonals, useCache)
    -- Overrides
    if useDiagonals == nil then useDiagonals = self.useDiagonals end
    if useCache == nil then useCache = self.useCache end
  ...
  internal.getCache = function(pathfinder, startX, startZ, endX, endZ)
    if pathfinder.cached ~= nil then
      ...
      if pathfinder.cached[pathIndex] ~= nil then return pathfinder.cached[pathIndex] else return nil end
    else return nil end
  end
  ...
  internal.calculateScores = function(node, endNode)
    if node.parent ~= nil then node.g = internal.elapsed(node.parent) else node.g = 0 end
