...
mod.generate = function(self, config)
	...
	local defaultConfig = {
		context = 'All prompts are supposed to describe ambiences, always consider that true, even if absurd. Reply with a JSON object, replacing values in this template (noon, clear sky): \n\
...
Sun rotation represents rotations around Y and X axis, in that order, in radians. (Y = 0 means sun is south, Y = pi / 2 means sun is west)',
	...
	aiChat:Say(config.prompt, function(err, response)
		...
		-- trim spaces and surrounding backticks
